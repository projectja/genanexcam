...
def update_spreadsheet(path, _df=CF, startcol:int=1, startrow:int=1, sheet_name:str ="Alta_Usuario", numFila:int=1):
    ...
    for row in range(0, _df.shape[0]): #For each row in the dataframe
        for col in range(0, _df.shape[1]): #For each column in the dataframe
            ...
            # estructura del _df
            #[razons,estadonif,nifempresa,fecha,tratamiento,nombre,primerapellido,segundoapellido,nif,cargo,correoelectronico]],columns=['Razón Social','Estado NIF','documento_solicitante','Fecha (DD/MM/AA)','Tratamiento','Nombre','Primer Apellido','Segundo Apellido','NIF','Cargo','Correo electrónico'])    
            ...
            #columns=['Razón Social','Estado NIF','documento_solicitante','Fecha (DD/MM/AA)','Tratamiento','Nombre',
...
def modify_data_pandas(cif_a_buscar,data_oap_tab_alta_usuario):    
   ...
   ######### df_tab_empresa_fila_buscada = data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.loc[data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME['documento_solicitante'] == cif_a_buscar]   
    ...
    df_tab_AltaUsuario_fila_modificada_o_nueva = pd.DataFrame([[razons,estadonif,nifempresa,fecha,tratamiento,nombre,primerapellido,segundoapellido,nif,cargo,correoelectronico]],columns=['Razón Social','Estado NIF','documento_solicitante','Fecha (DD/MM/AA)','Tratamiento','Nombre','Primer Apellido','Segundo Apellido','NIF','Cargo','Correo electrónico'])    
    ...
    #print(data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME[['documento_solicitante','Nombre o razón social']])
    ...
    if cif_a_buscar in data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.values and cif_a_buscar not in data_oap_tab_alta_usuario.values:
        ...
        """  df_tab_AltaUsuario_fila_modificada_o_nueva.at[numero_fila_dfAltaUsuario,'documento_solicitante'] = data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.at[numero_fila_dfAltaEmpresa,'documento_solicitante']
        ...
        if cif_a_buscar in data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.values and cif_a_buscar in data_oap_tab_alta_usuario.values:        
            ...
            # df_tab_AltaUsuario_fila_modificada_o_nueva.at[numero_fila_dfAltaUsuario, 'documento_solicitante'] = df_tab_empresa_fila_buscada.at[numero_fila_dfAltaEmpresa,'documento_solicitante'] 
            # print("hola")
            ...
            #df_tab_AltaUsuario_fila_modificada_o_nueva.at['0','documento_solicitante']= data_oap_tab_alta_usuario.at['0','documento_solicitante']
            #df_tab_AltaUsuario_fila_modificada_o_nueva.at['0', 'documento_solicitante'] = df_tab_empresa_fila_buscada.at[numero_fila_dfAltaEmpresa,'documento_solicitante'] 
                        
        # df_tab_AltaUsuario_fila_modificada_o_nueva = data_oap_tab_alta_usuario.at['0','documento_solicitante'] = 111
...
def leer_datos(df_tab_LISTA_ASESORE_GENERACION_DOCUMEs):
    df_tab_empresa_fila_buscada = df_tab_LISTA_ASESORE_GENERACION_DOCUMEs.loc[df_tab_LISTA_ASESORE_GENERACION_DOCUMEs['documento_solicitante'] == cif_a_buscar]   
    ...
    poblacion = df_tab_empresa_fila_buscada.iloc[0]['poblacion']
    # data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME =pd.read_excel('LISTA-ASESORE-GENERACION-DOCUMENTOS-2022-BAK-ANTES-DE-ANEXO-20-Y-21-AÑO-2023-V3.xlsx', sheet_name='LISTA-ASESORE-GENERACION-DOCUME', usecols = 'A:GO',header = 0, converters= { 'fecha_documento_inicio_diagnostico': pd.to_datetime, 'fecha_diagnostico': pd.to_datetime, 'cp': pd.to_numeric, 'telefono_solicitante': pd.to_numeric})
    cp = df_tab_empresa_fila_buscada.iloc[0]['cp'].to_numeric()
    global email    
    email = df_tab_empresa_fila_buscada.iloc[0]['email']
    direccion = df_tab_empresa_fila_buscada.iloc[0]['direccion']
    telefono_solicitante = df_tab_empresa_fila_buscada.iloc[0]['telefono_solicitante']
    ...
    context = {
                ...
                'nif_de_empresa' : cif_empresa,
                'nombre_representante' : nombre_representante.lower().title(),
                'razon_social_global' : nombre_o_razon_social       , #revisar si intruso
                'tecnico_justificar' :  tecnico_justificar,
