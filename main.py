#procbuscarcif_api # -*- coding: utf-8 -*-
...
async def lifespan(application: FastAPI):
    ...
    data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME = pd.DataFrame(sheet1)
    ...
    new_header = data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.iloc[0] #grab the first row for the header
    data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME = data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME[1:] #take the data less the header row
    data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME.columns = new_header #set the header row as the df header
    ...
    #book = ""
    #data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME = pd.read_excel('LISTA-ASESORE-GENERACION-DOCUMENTOS-2022-BAK-ANTES-DE-ANEXO-20-Y-21-AÑO-2023-V3.xls', sheet_name='LISTA-ASESORE-GENERACION-DOCUME', usecols = 'A:GO',header = 0)
    ...
    ##data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME = book.sheets['LISTA-ASESORE-GENERACION-DOCUME']
    ...
    #df2 = pd.DataFrame(pestana_LISTA_ASESORE_GENERACION_DOCUME)
...
def procbuscarcif(cifBuscar):
    ...
    fecha_inicio_diagnostico_formateada = fecha_documento_inicio_diagnostico.strftime("%d/%m/%Y")
    fecha_fin_diagnostico_formateada = fecha_diagnostico.strftime("%d/%m/%Y")
    segui_1_formateado = segui_1.strftime("%d/%m/%Y")

    print (' FECHA FORMATEADA '   + segui_1_formateado )
...
#    cp = str(math.trunc(cp))
    ...
    print('telefono ' + str(telefono_solicitante))
    ...
    print (' FECHA FORMATEADA seg 1 '   + str(segui_1 ))    
    print (' FECHA FORMATEADA seg 2 '   +  str(segui_2 )    )
    print (' FECHA FORMATEADA seg 2 '   +  str(segui_3 )    )
    print (' FECHA FORMATEADA seg 4 '   +  str(segui_4 )    )
    print (' FECHA FORMATEADA seg 5 '   +  str(segui_5 )    )
    print (' FECHA FORMATEADA seg 6 '   + str(segui_6)    )
    ...
    a = input ( ' pulsa una tecla ')
    dni_formateado = documento_representante[:2] + "." + documento_representante[2:5] + "." + documento_representante[5:8] + "-" + documento_representante[8]
    print ( " DNI FORMATEADO api " + dni_formateado)
    A = input( "PULSE UNA TECLA ....")
...
# a continuación tenemos las rutas, que fefinimos usando get post put...
# app es el nombre de la instancia de FASTAPI
# tenemso un decorador @ que lo que hace es registrar la función siguiente que viene a continuación la función salir
# con el decorador, registra la función y dice que cuando alguien llame a esa ruta, se ejecute la función salir
# ejecutar la isntanacia de fastapi usando 
# uvicorn trabajarexcel_api:app --reload
@app.get("/cif/{cif}")
def salir(cif):    
    cifpar = cif       
    print("el cif es "+ cif)
    ...
    contextdatar = procbuscarcif_api(data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME,cifpar)
    
       # menu_principal(data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME,cifpar)
    #if __name__ == '__main__':
    #async def root():
        #uvicorn.run(f'trabajarexcel_api:app', host='localhost', port=8000)
    
    # return contextdatar
        

@app.get("/anexo19/{context}")
def anexo19(contexto):    
    #cifpar = cif
    #print("el cif es "+ cif)
    ...
    # menu_principal(data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME,cifpar)
    #if __name__ == '__main__':
    dirdatos = generar_ficha_anexo19(contexto)
          # menu_principal(data_oap_tab_LISTA_ASESORE_GENERACION_DOCUME,cifpar)
    #if __name__ == '__main__':
    #async def root():
        #uvicorn.run(f'trabajarexcel_api:app', host='localhost', port=8000)
    return {"xxxx mensaje",1}
